...
const PhimBo: React.FC<{
  ...
  const seoTitle = `Phim Bộ Hay ${currentYear} - Xem Phim Bộ Mới Nhất ${pageNum > 1 ? `- Trang ${pageNum}` : ''}`;
  ...
  const canonicalUrl = `${process.env.NEXT_PUBLIC_BASE_URL || 'http://127.0.0.1:3000'}/phim-bo${pageNum > 1 ? `?trang=${pageNum}` : ''}`;
  ...
  useEffect(() => {
    ...
    if (router.isReady) {
      const urlType = (router.query.loai as string);
      setTempFilters({
        country: (router.query["quoc-gia"] as string) || "ALL",
        type: urlType === "movie" ? "movie" : "tv",
        rank: (router.query["xep-hang"] as string) || "ALL",
        genres: (router.query["the-loai"] as string) || "ALL",
        versions: (router.query["phien-ban"] as string) || "ALL",
        years: (router.query.nam as string) || "ALL",
      ...
      if (urlType === "movie") {
        currentApiParams.type = "movie";
      }

      // Xóa các param không cần thiết
      delete currentApiParams.slug;
      delete currentApiParams.loai;

      fetchMoviesNow(currentApiParams); // Function is now defined
    }
  }, [router.isReady, router.query, fetchMoviesNow]);
  ...
  const applyFilter = () => {
    ...
    if (tempFilters.type === "movie") {
      delete queryObj["loai"]; // Xóa để trang kia nhận mặc định
  ...
  return (
    <>
      ...
      <div className='pb-28 pt-5 px-5 lg:px-6' ref={listRef}>
        <div className='text-2xl font-semibold text-white'>Phim bộ</div>
        <div className='mt-3'>
