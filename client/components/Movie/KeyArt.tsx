                        ...
                        <SwiperSlide key={movie._id}>
                            <Link href={`/phim/${movie.slug}`} className="flex flex-col h-full group">
                                ...
                                >
                                    ...
                                    <div
                                        className="relative w-full h-[530px] overflow-hidden"
                                        style={{
                                            clipPath: currentClipPath,
                                        }}
                                    >
                                        ...
                                        <div className="absolute bottom-0 left-0 justify-center w-full flex">
                                            {movie.lang && movie.lang.map((lang, i) => {
                                                // Kiểm tra i === 0 (Thẻ đầu tiên)
                                                const isFirst = i === 0;

                                                // Dùng switch/case cho dễ đọc hơn
                                                let content;
                                ...
                                <div className="flex w-full space-x-5 mt-3 items-center">
                                    ...
                                    <div className="flex flex-col space-y-2">
                                        <h3 className="text-sm text-white line-clamp-2 overflow-hidden text-ellipsis">
                                            {movie.name}
                                        </h3>
                                        <p className="text-xs text-gray-400">{movie.origin_name}</p>
                                        <div className="flex items-center space-x-2 rounded-lg bg-gray-900 ">
                                            <span className="text-xs font-bold text-white">{movie.episode_total}</span>
                                            <span className="text-gray-500">&bull;</span>
                                            <span className="text-xs text-gray-300">Phần {movie.tmdb?.total_seasons}</span>
                                            <span className="text-gray-500">&bull;</span>
