...
window.addEventListener("message", (event) => {
    ...
    if (data.action === 'play' && data.playlist) {

        if (typeof currentSkipConfig !== 'undefined') {

            currentSkipConfig = data.skip;

        } else {

            window.currentSkipConfig = data.skip;

        }



        // XỬ LÝ TRÙNG LẶP LABEL

        if (data.playlist && data.playlist.length > 0 && data.playlist[0].sources) {
            ...
            sources.forEach(source => {
                ...
                if (labelCounts[label] > 1) {
                    ...
                    if (!labelTracker[label]) labelTracker[label] = 1;

                    source.label = `${label} #${labelTracker[label]}`;
            ...
            currentPlaylistSources = sources;

            currentSourceIndex = sources.findIndex(s => s.default);
                        ...
                        <div class="flex items-center justify-between gap-2 md:gap-4 lg:gap-8 w-full">
                            ...
                            <div class="inline-flex items-center gap-2 md:gap-4 lg:gap-6 shrink-0">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <button id="toggle_play" class="h-[27px] w-[27px] md:h-[37px] md:w-[37px] lg:h-[48px] lg:w-[48px] text-white hover:text-primary transition-colors">

                                        <svg fill="none" viewBox="0 0 24 24" class="w-full h-full" xmlns="http://www.w3.org/2000/svg"><path d="m10.8 15.9 4.67-3.5c.27-.2.27-.6 0-.8l-4.67-3.5c-.33-.25-.8-.01-.8.4v7c0 .41.47.65.8.4zm1.2-13.9c-5.52 0-10 4.48-10 10s4.48 10 10 10 10-4.48 10-10-4.48-10-10-10zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm2-4c.55 0 1-.45 1-1v-6c0-.55-.45-1-1-1s-1 .45-1 1v6c0 .55.45 1 1 1z" fill="currentColor"></path></svg>
