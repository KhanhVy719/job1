...
class TiktokService {
  ...
  public async processJob(
    ...
    const buildArgs = (duration: number) => [
      ...
      "-profile:v", "main", "-crf", "23", "-g", String(Math.max(1, Math.round(duration * 6))),
    ...
    try {
      ...
      for (const item of uploadedData) {
        // uploadToTiktokAPI luôn trả về URL công khai đầy đủ (https://...)
    ...
    return new Promise((resolve, reject) => {
      ...
      ps.on("close", (code) => {
        ...
        try {
          ...
          // Bitrate (bps) để chia segment theo dung lượng MB.
          // Ưu tiên format.bit_rate (tổng), fallback stream.bit_rate, cuối cùng
          // ước lượng từ size/duration nếu cả hai thiếu.
          let bitrate = parseFloat(format?.bit_rate || "0");
          if (!(bitrate > 0)) bitrate = parseFloat(stream?.bit_rate || "0");
          const duration = parseFloat(format?.duration || "0");
          if (!(bitrate > 0) && duration > 0) {
            const size = parseFloat(format?.size || "0");
            if (size > 0) bitrate = (size * 8) / duration;
          }
