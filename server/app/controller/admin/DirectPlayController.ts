...
async function rateLimit(req: Request, scope: string, max = 240): Promise<boolean> {
  const result = await checkFixedWindowRateLimit({
    scope: `direct-play:${scope}`,
    identity: clientFingerprint(req),
    max,
    windowSeconds: 60,
  });
...
class DirectPlayController {
  ...
  playlist = async (req: Request, res: Response) => {
    if (!(await rateLimit(req, "playlist", 60))) return res.status(429).send("rate limited");
  ...
  segmentMeta = async (req: Request, res: Response) => {
    if (!(await rateLimit(req, "segment-meta", 600))) return res.status(429).json({ status: false, message: "rate limited" });
    const token = verifyToken(req, req.query);
  ...
  key = async (req: Request, res: Response) => {
    if (!(await rateLimit(req, "key", 120))) return res.status(429).send("rate limited");
    const token = verifyToken(req, req.query);
