...
class LocalTokenService {
  ...
  constructor() {
    // Bắt buộc lấy từ ENV. Không có fallback hardcode để tránh lộ secret.
    if (!process.env.TOKEN_MASTER_SECRET) {
      throw new Error("FATAL: TOKEN_MASTER_SECRET is not set in environment.");
    }
