...
class UploadJobQueue {
  ...
  private async process(jobId: string) {
    ...
    try {
      ...
      await this.forceProgress(job.job_id, 5, "Analyzing video format...", "processing");
      ...
      const result = await TiktokService.processJob(
        ...
        metadata.duration,
        (percent, message) => {
          this.writeProgress(job.job_id, percent, message, "processing");
        },
        metadata.bitrate,
  ...
  private async cleanupFile(filePath?: string) {
    ...
    try {
      const allowedTmp =
        filePath.startsWith(this.tmpDir) || filePath.startsWith(this.multerTmpDir);
      if (fs.existsSync(filePath) && allowedTmp) {
